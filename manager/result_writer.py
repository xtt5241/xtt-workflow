...
def build_result_payload(task_path: Path, log_path: Path | None = None, result_path: Path | None = None) -> dict:
    ...
    payload = {
        ...
        "required_validation_actions": [str(item).strip() for item in task.get("required_validation_actions", []) if str(item).strip()],
        "high_risk_residuals": [str(item).strip() for item in task.get("high_risk_residuals", []) if str(item).strip()],
        "risk_check_summary": str(task.get("risk_check_summary", "")).strip(),
        "risk_check_report": maybe_keep(task.get("risk_check_report", {}), existing.get("risk_check_report")),
